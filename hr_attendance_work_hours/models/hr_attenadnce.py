...
class HRAttendance(models.Model):
    ...
    def _compute_worked_hours(self):
        ...
        for attendance in self:
            if attendance.check_out and attendance.check_in and attendance.employee_id:
                ...
                #                                                                             lunch=True)
                # att = Intervals([(check_in_tz, check_out_tz, attendance)])
                # - lunch_intervals
    ...
    def _compute_attendance_work_hours(self):
        for rec in self:
            if rec.employee_id and rec.check_in and rec.check_out:
                calendar = rec._get_employee_calendar()
                hours = rec.employee_id._get_work_days_data_batch(rec.check_in, rec.check_out, calendar=calendar)
                ...
                check_out_tz = rec.check_out.astimezone(tz)
                lunch_intervals = rec.employee_id._employee_attendance_intervals(check_in_tz, check_out_tz,
                                                                                            lunch=True)
                attendance_intervals = lunch_intervals
                delta = sum((i[1] - i[0]).total_seconds() for i in attendance_intervals)
                rec.out_work_hours = rec.worked_hours-rec.in_work_hours-(delta/3600) if rec.worked_hours-rec.in_work_hours-(delta/3600) >= 0 else 0
            else:
                rec.in_work_hours = 0
    ...
    def _compute_late_early_hours(self):
        for rec in self:
            if rec.employee_id and rec.check_in and rec.check_out:
                calendar = rec._get_employee_calendar()
                tz = rec.employee_id.tz or calendar.tz
                ...
                for day in days_from_hours.filtered(lambda x:x.day_period != 'lunch'):
                ...
                local_date_to = local_timezone.localize(rec.check_out).astimezone(timezone(tz))
                ...
                early_checkout = 0
                flag = True
                for day in days_to_hours.filtered(lambda x:x.day_period != 'lunch'):
